    ...
    try {
      ...
      for (const item of items) {
        if (
          ...
          return res.status(400).json({
            message:
              "Each item requires product, quantity, and unit cost.",
          });
        }
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          message:
            "A received purchase order cannot be changed to this status.",
        });
      }
    ...
    try {
      ...
      for (const receivedItem of items) {
        ...
        if (
          ...
          return res.status(400).json({
            message:
              `Cannot receive more than the remaining quantity for ${orderItem.product}.`,
          });
        }
      }
