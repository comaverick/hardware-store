...
disabled; incompatible frames are still rejected. `algorithmVersion: 14`
...
Algorithm 14 has no single-view or registered-composite fallback. Room captures below
...
single frame. Surface completion also requires one dominant wall direction,
one consistent wall-depth layer, and adequate measured coverage across a 2D
wall grid. It uses a finer, wall-sized fusion volume; wide multi-wall sectors
remain in capture instead of being emitted as a low-resolution surface.
