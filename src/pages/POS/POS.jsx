...
const POS = () => {
  ...
  const [printerLoading, setPrinterLoading] =
    useState(true);
  const [printerOnline, setPrinterOnline] =
    useState(false);
  ...
  const addToCart = (product) => {
    ...
    if (available <= 0) {
      message.warning(
        `${product.name} is out of stock in this branch.`
      );
      return;
    }
    ...
    if (existing) {
      if (
        ...
        message.warning(
          `Only ${available} available in this branch.`
        );
        return;
      }
  ...
  const printReceipt = async (sale) => {
    try {
      ...
      message.warning(
        "Sale completed, but the receipt could not be printed."
      );
